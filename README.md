...
- 支持通过环境变量配置运营商动态分配的 IPv6 前缀，避免将局域网 IPv6 设备间通信误计为公网流量
- 自动检测本机网卡绑定的所有 IPv6 地址，正确区分"NAS 向外发送文件"（上行）与"外部向 NAS 下载"（下行）
- 本机 IPv6 地址每 10 分钟自动刷新一次，应对 SLAAC 隐私扩展地址轮换
...
│  └── GET /api/debug/local_ips 本机 IP 调试               │
│            │                                             │
└────────────┼────────────────────────────────────────────┘
             │ HTTP
             ▼
...
程序以多线程方式运行，四个核心线程职责如下：

| 线程名 | 职责 |
|--------|------|
| `capture`（主线程） | 原始套接字收包循环，解析帧并统计流量 |
| `tick` | 每秒快照一次当前速率，维护最近 120 秒的速率窗口 |
| `ip-refresh` | 每 10 分钟重新检测网卡绑定 IP，应对 IPv6 地址轮换 |
...
编辑 `docker-compose.yml`，至少修改 `MONITOR_IFACE`：

```yaml
environment:
  - MONITOR_IFACE=enp2s0        # ← 改为你的实际网卡名
  - EXCLUDE_IPV6_PREFIX=        # ← 如有运营商 IPv6 前缀，填在这里（见下文）
...
| `EXCLUDE_IPV6_PREFIX` | `""` | 可选 | 需额外排除的 IPv6 前缀，多个用英文逗号分隔 |
...
大部分的家用 NAS 是没有公网 IPv4的（在路由器 NAT 之后），但大多数运营商现在为每个家庭分配了公网 IPv6 地址段，家里的每台设备（NAS、电脑、手机）都会获得一个公网 IPv6 地址。

由于 IPv6 不存在 NAT，当**局域网内的手机通过 IPv6 访问 NAS** 时，数据包的源地址和目的地址都是公网 IPv6，程序必须通过本机绑定的 IPv6 地址列表来判断哪端是 NAS 本身，进而确定方向。

**特殊场景需要配置 `EXCLUDE_IPV6_PREFIX`：**

如果你局域网内有设备通过 IPv6 与 NAS 通信，且这部分流量**不应算作公网流量**（例如家里的 NAS 和 PC 通过 IPv6 互相传文件），则需要将运营商分配给你的 IPv6 前缀加入排除列表。
...
# 你的前缀通常取前 3~4 段：240e:33e:2f08:d600::/56
```
...
容器运行后访问调试接口，确认 NAS 的公网 IPv6 出现在 `ipv6` 列表中：
...
```json
{
  "iface": "eth0",
  "ipv4": ["192.168.1.100"],
  "ipv6": ["240e:33e:2f08:d601::1", "240e:33e:2f08:d601:xxxx:yyyy:zzzz:aaaa"],
  "total": 3,
  "note": "上行/下行方向判断依赖此列表..."
}
```

如果 NAS 的公网 IPv6 地址出现在这里，说明方向判断将正确工作。
...
返回程序当前检测到的本机 IP 列表，用于验证 IPv6 方向判断是否正确。
...
健康检查接口，供 Docker healthcheck 使用。

```json
{ "status": "ok", "ts": "2024-09-15T14:23:01.234567" }
```
...
CREATE TABLE traffic_hourly (
    ...
    hour_ts    TEXT NOT NULL UNIQUE,   -- 'YYYY-MM-DD HH:00:00'
...
流量数据首先在内存中按小时粒度累加（`defaultdict`），每隔 `SAVE_INTERVAL` 秒批量写入。写入采用 `INSERT ... ON CONFLICT DO UPDATE SET ... = ... + excluded.` 幂等语句：

```sql
INSERT INTO traffic_hourly (hour_ts, up_bytes, down_bytes)
VALUES ('2024-09-15 14:00:00', 1234, 5678)
ON CONFLICT(hour_ts) DO UPDATE SET
    up_bytes   = up_bytes   + excluded.up_bytes,
    down_bytes = down_bytes + excluded.down_bytes;
```

**即使因断电或异常导致同一小时数据被写入多次，也只会在已有数值上继续累加，不会产生重复统计。**
...
| `ff00::/8` | 组播地址 |
| `::1/128` | 本机回环 |
| `EXCLUDE_IPV6_PREFIX` 配置项 | 用户自定义排除前缀（运营商 IPv6 段） |
...
- IPv6：属于 fe80::/10、fc00::/7 等 → 内网侧；**在本机绑定的 IPv6 列表中 → 本机侧**；否则为远端公网
...
nettraffic-sentinel/
│
├── app.py              # 主入口：启动抓包、持久化、Flask 三个线程
├── capture.py          # 抓包核心：raw socket、IP 解析、方向判定、内存统计
├── database.py         # 数据层：SQLite WAL、小时存储、多维度查询
├── api.py              # HTTP API：Flask 路由、实时汇总、日期范围查询
│
├── static/
...
├── Dockerfile          # 镜像：python:3.11-slim + ethtool + iproute2
...
`capture.py` 是整个系统最核心的文件，承担：原始帧接收 → 协议解析 → IP 归属判定 → 方向判定 → 线程安全统计 → 实时速率采样 → 本机 IP 定期刷新，所有统计精度相关的逻辑都在这里。

`database.py` 负责持久化，设计上保证任意时刻重启都不会丢失已统计的数据，也不会产生重复计数。天和月级别的统计不单独存储，而是通过 SQL 视图从小时表实时聚合，避免数据不一致。
...
访问 `/api/debug/local_ips` 查看程序检测到的本机 IP 列表。如果 NAS 的公网 IPv6 不在列表中，说明 `MONITOR_IFACE` 配的网卡名与实际拥有该 IPv6 地址的网卡不一致。
